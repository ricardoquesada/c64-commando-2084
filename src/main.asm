...
p2B = $2B
p5D = $42                       ;Used by music
p5E = $43                       ;Used by music
p5F = $44                       ;Used by music
p60 = $45                       ;Used by music
pF7 = $F7
pF8 = $F8
...
_L04    STY pFD       ;Store current row idx
RUN_ACTION
        LDA (p28),Y
        ASL A
        TAY
...
;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;
; When hero's grenade hit the turret, more precisely the cannon, the turret
; gets destroyed
DESTROY_TURRET
        TXA
        PHA
...
_L00    PLA
        ...
        PLA
        TAX
        JMP b3793               ;A grenade could kill more than one enemy at the
                                ; time. Continue iterating over the enemies.
...
;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;
; Original music was placed at $5000 but we can place it in an place.
;        *= $5000
.include "music.asm"
